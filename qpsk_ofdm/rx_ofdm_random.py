...
# 建立一個索引列表，方便繪圖
data_carrier_indices = np.hstack([
    np.arange(data_left_start, data_left_end),
    np.arange(data_right_start, data_right_end)
])
plot_indices = np.arange(num_data_carriers) # 0 到 47
...
try:
    while True:
        ...
        for i in range(N_DATA_SYMBOLS):
            ...
            # (!!! 關鍵修改 !!!)
            # 從「帶有 DC 缺口」的子載波中提取數據
            payload_left = data_freq_equalized[data_left_start:data_left_end]
            payload_right = data_freq_equalized[data_right_start:data_right_end]
            payload = np.hstack([payload_left, payload_right])
        ...
        err_pow = np.mean(np.abs(gt_payload_freq - rx_payload_freq)**2) # 這就是 MSE
        esnr = 10*np.log10(sym_pow / err_pow)
        
        print(f"\n--- FINAL RESULT ---")
        print(f"--- ESNR: {esnr:.2f} dB ---")
        print(f"--- MSE:  {err_pow:.4e} ---") # (!!! 新增 !!!)
        ...
        ax1.plot(plot_indices, channel_mag_used, '.-')
        ax1.set_title(f'Channel Magnitude (H) - Used Carriers')
        ax1.set_xlabel('Data Subcarrier Index (0-47)')
        ...
        # (!!! 關鍵修改 !!!) 畫出我們避開的 DC 區域
        ax2.axvspan(-Fs/FFT_size*(DC_NULLS/2), Fs/FFT_size*(DC_NULLS/2), 
                    color='red', alpha=0.2, label=f'DC Null Zone ({DC_NULLS} carriers)')
        ax2.legend()
